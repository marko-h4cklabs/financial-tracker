...
import { ArrowLeft, Edit, Plus, CheckCircle, Repeat } from 'lucide-react'
import { format, addMonths, differenceInMonths } from 'date-fns'
import { supabase } from '@/lib/supabase'
import { logActivity } from '@/lib/activityLogger'
import { formatCurrency, formatDate } from '@/lib/formatters'
import { useAuth } from '@/store/authStore'
import type { Deal, Client, Profile, Installment, Expense, DealStage } from '@/types'
import Button from '@/components/ui/Button'
import Card from '@/components/ui/Card'
import { DealStageBadge, InstallmentStatusBadge, CategoryBadge } from '@/components/ui/Badge'
...
type Tab = 'overview' | 'installments' | 'expenses'
...
export default function DealDetailPage() {
  ...
  const fetchDeal = useCallback(async () => {
    if (!id) return
    setLoading(true)
    const [{ data: d }, { data: inst }, { data: exp }, { data: c }, { data: p }] = await Promise.all([
          ...
          <div className="space-y-3">
            ...
            <Card className="p-4">
              ...
              <p className="text-xl font-light" style={{ fontFamily: 'DM Mono, monospace', color: 'var(--status-green)' }}>
                {formatCurrency(paidInstallments, deal.currency)}
              </p>
            </Card>
            <Card className="p-4">
              <p className="text-xs uppercase tracking-wider mb-1" style={{ color: 'var(--text-muted)' }}>Remaining</p>
              <p className="text-xl font-light" style={{ fontFamily: 'DM Mono, monospace', color: 'var(--status-yellow)' }}>
                {formatCurrency(totalValue - paidInstallments, deal.currency)}
              </p>
            </Card>
          </div>
        </div>
